 ...
 * محرك التتبع والتفاعل فائق السرعة
...
const CONFIG = {
  clientPhone: '0500434097',
  clientPhoneIntl: '966500434097',
...
// 4. معالجة نقرات الاتصال الهاتفي (مع استثناء رقم المطور)
...
// 5. معالجة نقرات الواتساب (مع استثناء رقم المطور)
...
// 6. تشغيل القائمة المتنقلة للجوال (Mobile Hamburger Menu)
...
// 8. حاسبة الأسعار التفاعلية للمظلات والسواتر والساندوتش بانل والبرجولات
function setupPriceCalculator() {
  const calcForm = document.getElementById('priceCalculatorForm');
  if (!calcForm) return;
  ...
  function calculate() {
    ...
    const pricePerUnit = parseFloat(serviceSelect.value) || 0;
    ...
    if (sendWhatsappBtn) {
      ...
      const message = `السلام عليكم مؤسسة العتيق، أود الاستفسار عن تفاصيل وحجز مقايسة بالرياض:%0A- نوع الخدمة: ${selectedText}%0A- المساحة / الأبعاد التقديرية: ${area} متر مربع%0A- التكلفة التقديرية بالحاسبة: ${total} ريال سعودي`;
...
function setupSmartLeadForms() {
  ...
  forms.forEach((form) => {
    form.addEventListener('submit', (e) => {
      ...
      const service = serviceSelect ? serviceSelect.value : 'طلب خدمات حدادة ومظلات';
      const district = districtInput ? districtInput.value.trim() : 'مدينة الرياض';

      const message = `طلب معاينة وتركيب جديد من الموقع الإلكتروني:%0A- الاسم: ${name}%0A- الجوال: ${phone}%0A- الخدمة المطلوبة: ${service}%0A- الحي / الموقع بالرياض: ${district}`;
...
// 10. تشغيل أكورديون الأسئلة الشائعة
...
// 11. تهيئة المستمعات بعد تحميل الـ DOM
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ربط جميع أزرار الاتصال والواتساب بالتحويل التلقائي
